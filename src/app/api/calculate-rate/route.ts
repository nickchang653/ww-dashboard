...
export async function POST(request: NextRequest) {
  ...
  for(let index = 0; index < years; index ++) {
    ...
    let spBalance = index == 0 ? (investment * 1.0 * spRate / 100): history[index - 1].sp.balance * (1 + ((SP_500_GROWTHS as any)[(year - 1).toString()])/100) - wdMoney;
  ...
  let spBalance = history[years - 1].sp.balance * (1 + ((SP_500_GROWTHS as any)[(year - 1).toString()])/100) - wdMoney;
