...
export async function POST(request: NextRequest) {
  ...
  for(let index = 0; index < years; index ++) {
    ...
    let spBalance = index == 0 ? (investment * 1.0 * spRate / 100): history[index - 1].sp.balance * (1 + ((SP_500_GROWTHS as any)[(year - 1).toString()] - spFee)/100) - wdMoney;
    let spGrowth = (SP_500_GROWTHS as any)[(year).toString()]
    let spWdMoney = wdMoney;

    let inParBalance = index == 0 ? (investment * 1.0 * inParRate / 100): history[index - 1].inPar.balance * (1 + (history[index - 1].inPar.growth - inParFee) / 1.0 / 100) - inParWdMoney;
    ...
    let inParBonusBalance = index == 0 ? (investment * 1.0 * inParBonusRate / 100): history[index - 1].inParBonus.balance * (1 + (history[index - 1].inParBonus.growth - bonusFee) / 1.0 / 100 ) - inParWdMoney;
    let inParBonusGrowth = inParBonus;
    inParBonusGrowth = inParBonusGrowth > 0 ? inParBonusGrowth: 0;

    let snBalance = index == 0 ? (investment * 1.0 * snRate / 100): history[index - 1].sn.balance * (history[index - 1].sn.growth - frFee)  / 1.0 / 100 +  history[index - 1].sn.balance - snWdMoney;
  ...
  let spBalance = history[years - 1].sp.balance * (1 + ((SP_500_GROWTHS as any)[(year - 1).toString()] - spFee)/100) - wdMoney;
  let inParBalance = history[years - 1].inPar.balance * (1 + (history[years - 1].inPar.growth - inParFee) / 1.0 / 100) - inParWdMoney;
  let inParBonusBalance = history[years - 1].inParBonus.balance * (1 + (history[years - 1].inParBonus.growth - bonusFee) / 1.0 / 100 ) - inParWdMoney;
  let snBalance = history[years - 1].sn.balance * (history[years - 1].sn.growth - frFee)  / 1.0 / 100 +  history[years - 1].sn.balance - snWdMoney;
